...
var row = getRowCount();
//console.log(row);
console.log("row class의 개수는 " + row +"개 입니다." )
...
5. 4 경우에서 경고창을 확인하면 enterName 함수를 다시 호출합니다me).
...
//enterName();
var enterName = prompt('이름을 입력해 주세요')

    if(enterName !== null){ //확인누름

        if(enterName === ''){ //내용입력을 안했을때
            var alert = alert('이름을 정확히 입력해주세요.');  

                if(alert !== null){ //경고창에서 확인을 눌렀을때
                    var enterName = prompt('이름을 입력해 주세요')
                }
        }
    }
