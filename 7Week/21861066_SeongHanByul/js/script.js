...
btn_window_size.addEventListener('click', function(evt){
...
2. id 가 'user_id' 인 요소를 찾고 value 속성의 값의 문자열 개수를 아래의 문자열 결과값과 같이 console 에 출력되도록 작성합니다.
    ...
    못품
*/
//var btn_check_id = document.getElementById("btn-check-id");
var user_id = document.getElementById('user_id');
var len = user_id.attributes['value'];
console.log(len);
console.log(len.length);

// btn_check_id.addEventListener('click', function(evt){
//     var user_id = document.getElementById('user_id')
//     console.log(user_id);
//     var value = user_id.nodeValue;
...
btn_cancel.addEventListener('click', function(evt){location.reload();});
...
btn_apply.addEventListener('click', function(evt){
...
btn_add_city.addEventListener('click', function(evt){

    var city = prompt('도시명을 입력하세요');
    if(city !== null){
        var city_list = document.getElementById("city-list");
        console.log(city_list.childNodes);
    }
})
