...
// $btnWindowSize.addEventListener("click",function(){
...
function onClickBtnCheckId(event){
    event.preventDefault(); //form 의 자식 요소로  button 이 있을 때는, form 전송 기능이 발생한다.
    //기본 기능을 막아주기 위해 클릭 이벤트가 호출되었을 때 preverntDefault(); 라는 이벤트 메서드(명령어)로 기본 기능을 막아줘야 함
    var $userId = document.getElementById("user_id");
    var value = $userId.value;
    console.log(value); //현재 input 에서 입력값이 바뀌면 그 값을 반환
    // value = $userId.getAttribute("value");
    // console.log(value); // html 로드 된 문서 상의서의 값을 반환
    //value = string type
    //문자열의 개수.lentgh;
...
function onClickBtnReset(event){
    ...
    // parseInt("100",10); //10진수로 변환
    if(Number(number) === 100){
        var $userId = document.getElementById("user_id"),
            $userPass = document.getElementById("user_pass");
        $userId.value = "";
        $userPass.value = ""; //null 써도 됨
