...
var $btn_1 = document.getElementById("btn-window-size");
$btn_1.addEventListener("click", function(event){
    alert("윈도우 콘텐츠의 영역 width : " + width + "px, height : " + height + "px 입니다.");
})
...
var $btn_2 = document.getElementById("btn-check-id");
var us_id = document.getElementById("user_id");
$btn_2.addEventListener("click",function(event)
{
    console.log("input#user_id - value 속성 값의 개수는" + us_id +"입니다.");
})
...
var $btn_3 = document.getElementById("btn-cancel");
...
var $btn_4 = document.getElementById("btn-apply");
$btn_4.addEventListener("click", function(event){
...
*/
var $btn_5 = document.getElementById("btn-reset");
$btn_5.addEventListener("click",function(event){
    var pro = prompt("초기화하시려면 100을 입력하세요.")
    if(pro==100){
        
        var input_1 = document.getElementById("user_id");
        var input_2 = document.getElementById("user_pass");
    
        clearInterval(input_1);
        clearInterval(input_2);
    }
})
...
var $btn_6 = document.getElementById("btn-add-city");
var $list = document.getElementById("city-list");
$btn_6.addEventListener("click", function(event){
...
4. button#btn-color 가 여러번 클릭되어 마지막 p 요소가 예외처리 된 상황(글자 색 스타일 'black')에서 button#btn-color 를 다시 클릭하게 되면 처음 위치한 
p 요소부터 예외처리 되도록 작성합니다.
...
var $btn_7 = document.getElementById("btn-color");
var $red = document.getElementsByClassName("sentence");
var $p = document.getElementsByClassName("section");
var _cuId = 0;
// console.log($AllSen); // sentence를 담고 있는 class 명 선언하여 처음에 클릭 시 빨간색으로 바꿔준다.
$btn_7.addEventListener("click", function(event){
    $p[0].style.color = "red";

    function color(_cuid){

    if(_cuId%5 ==1){ 
        $red[0].style.color = "black";
    }
    else if(_cuId%5 == 2){
        $red[1].style.color = "black";
    }
    else if(_cuId%5 == 3){
        $red[2].style.color = "black";
    }
    else if(_cuId%5 == 4){
        $red[3].style.color = "black";
    }
...
*/
var $btn_8 = document.getElementById("btn-go-top");
$btn_8.addEventListener("click", function(event){
    window.scrollTo(0,0)})