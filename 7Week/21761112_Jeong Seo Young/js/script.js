...
*/
var $btn = document.getElementById("btn-window-size");
$btn.addEventListener("click",click$btn);
var w = window.innerWidth;
var h = window.innerHeight;
function click$btn(event){
    alert("윈도우 콘텐츠의 영역 width : "+ w +"px,  height : " + h + "px 입니다." );
}

//console.log(w);
//console.log(h);  확인차 사용.
...
/*
 var $btn_1 = document.getElementById("btn-check-id");
 $btn_1.addEventListener("click",click$btn_1);
 var $id = document.getElementById("user_id").attributes("value");
function click$btn_1(event){
     console.log("input#user_id - value 속성 값의 개수는" + $id.lenght + "입니다.");
}
*/ //시도해보았으나 계속 된 오류에 실패....^^;;;
...
$btn_apply.addEventListener("click",$join);
function $join(event){
...
function $reset(event){
      var $100 = prompt('초기화하시려면 100을 입력하세요.')
      if($100 == 100){
        
      };
};
*///더이상 범접하면 제가 위험해질듯 하여 ^^;;;
...
*/
var $btn_add = document.getElementById("btn-add-city");
var $list = document.getElementById("city-list");

$btn_add.addEventListener("click", $city);

function $city(event){
    var $string = prompt("도시명을 입력하세요.");
    if($string !== null){
       var $newBox = document.createElement('li');
       $list.appendChild($newBox);
       //document.write( $newBox.$string); //오답...
       $newBox.innerText = $string;
    }else{
...
$color.addEventListener("click", $p);
var c = 0;
function $p (event){
    //과제제출 시간을 지키기 위해...풀던 중에 냅니다!
c++
}
...
*/
var $btn_top = document.getElementById("btn-go-top");

$btn_top.addEventListener("click",$gt);
function $gt(event){
    window.scrollTo(0,0);
}