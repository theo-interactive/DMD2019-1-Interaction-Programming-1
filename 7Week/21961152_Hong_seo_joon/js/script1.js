...
function onClickBtnCheckId(event) {
    ...
    //form의 자식요소로 button 이 있을 떄는 from  전송기능이 발생한다
    //기본 기능을 막아주기 위해 이벤트가 호출 되었을때
    //event.preventDefault();
    //라는 이벤트 메서드(명령어)로  기본 기능을 막아 주어야함
        var $userId = document.getElementById("user_id");
        var value = $userId.value;
        console.log(value); //현재 int 에서 입력값이 바뀌면 그 값을 반환.
        value = $userId.getAttribute("value");
        console.log(value); //html 로드 된 문서 상에서의 값을 반환.


        var result = "input#user_id - 속성값의 개수는" + value.length + "입니다.";  
        console.log(result);
}
// value = $userID.getAttribute("value");
...
/*
var buttonbtn_check_id =  document.getElementById('user id');

buttonbtn_check_id.addEventListener('click', function(event){
...
$btnCancel.addEventListener("click", onClickBtnCencel);
function onClickBtnCencel(event)
{
    event.preventDefault();
    window.locaton.reload(); //새로고침
...
{
    event.preventDefault();
    var number = prompt("초기화 하시려면 100을 입력하세요");
    //Number("100"); //문자를 숫자로 변환
...
$btnColor = document.querySelector("button#btn-color")
...
function onClickBtnColor(event){
    event.preventDefault();
        ...
        $sentence[i].getElementsByClassName.color = "red";
        if(count === i){
